...
Этот график показывает содержание GC по всей длине каждой последовательности в файле и сравнивает его с нормальным распределением. 

#### Предупреждение

Предупреждение выдаётся, если сумма отклонений от нормального распределения составляет более 15% от общего числа данных.

#### Ошибка

Ошибка выдаётся, если сумма отклонений составляет более 30% от общего числа данных.
...
Этот график показывает распределение размеров фрагментов в проанализированном файле.

#### Предупреждение

Предупреждение выдаётся, если все последовательности не имеют одинаковой длины.

#### Ошибка

Ошибка выдаётся, если какая-либо из последовательностей имеет нулевую длину.
...
График показывает относительное количество последовательностей с разной степенью дублирования.

#### Предупреждение

Этот модуль выдаст предупреждение, если неуникальные последовательности составляют более 20% от общего числа.

#### Ошибка

Этот модуль выдаст сообщение об ошибке, если неуникальные последовательности составляют более 50% от общего числа.
